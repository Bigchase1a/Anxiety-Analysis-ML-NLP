...
coefs = model.feature_log_prob_[1]
top_features = sorted(zip(coefs, tfidf.get_feature_names_out()), key=lambda x: x[0], reverse=True)[:20]
top_coefs = pd.DataFrame(top_features, columns=["Log-Olasılık", "Kelime"])

plt.figure(figsize=(10, 5))
sns.barplot(data=top_coefs, x="Log-Olasılık", y="Kelime", hue="Kelime", palette="viridis", legend=False)
